...
(defun sclang-compilation-parse-errors (limit-search find-at-least)
  ;; stub, until we start doing something with errors
)
...
(defun sclang-init-post-buffer ()
  ...
  (with-sclang-post-buffer
   ...
   (setq compilation-error-regexp-alist
	 (cons (list sclang-parse-error-regexp 2 3 4)
	       compilation-error-regexp-alist))
   (setq compilation-parse-errors-function
	(lambda (limit-search find-at-least)
	  (sclang-compilation-parse-errors limit-search find-at-least)))
